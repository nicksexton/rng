...
#define WM_RETRIEVAL_LATENCY_SCALE 3
...
double wm_retrieval_latency(OosVars *gv, long item)
{
  ...
  while (!(contents == NULL)) {
    ...
    if (item == wm_item) {
      ...
      retrieval_latency = (WM_RETRIEVAL_LATENCY_SCALE * exp(-1 * (mean_activation - activation)));
...
static Boolean check_random(OosVars *gv, long r)
{
  ...
  double r_latency = wm_retrieval_latency(gv, r);
...
// #if FALSE
	...
	long p1 = -1, p2 = -1, p3 = -1, p4 = -1; // init values - don't want to trigger random rules!
	double p1_latency, p2_latency, p3_latency, p4_latency;
	int g1 = -99, g2 = -99, g3 = -99, g4 = -99;
	long latest_timestamp = 0, latest_timestamp_but1 = 0, latest_timestamp_but2 = 0, latest_timestamp_but3 = 0;
	long temp_wm_value;
	...
	while (!(contents == NULL)) {
	  ...
	  /* contingency if r timestamp is latest */
	  if (contents->timestamp > latest_timestamp) {
	    pl_is_integer(pl_arg_get(contents->head, 1), &temp_wm_value);
	    ...
	    if (item_retrieved_from_wm(gv, contents->activation)) {
	      ...
	      latest_timestamp_but3 = latest_timestamp_but2;
	      latest_timestamp_but2 = latest_timestamp_but1;
	      latest_timestamp_but1 = latest_timestamp;
	      latest_timestamp = contents->timestamp;
	    }
	    else {
	  ...
	  /* contingency if r timestamp is latest but 1 */
	  else if (contents->timestamp > latest_timestamp_but1) {
	    pl_is_integer(pl_arg_get(contents->head, 1), &temp_wm_value);
	    if (item_retrieved_from_wm(gv, contents->activation)) {
	      p4 = p3;
	      p3 = p2;
	      ...
	      latest_timestamp_but3 = latest_timestamp_but2;
	      latest_timestamp_but2 = latest_timestamp_but1;
	      latest_timestamp_but1 = contents->timestamp;
	    }
	    else {
	      set_wm_item_last_retrieved (gv, temp_wm_value, FALSE);
	    }
	  ...
	  /* contingency if r timestamp is latest but 2 */
	  else if (contents->timestamp > latest_timestamp_but2) {
	    pl_is_integer(pl_arg_get(contents->head, 1), &temp_wm_value);
	    if (item_retrieved_from_wm(gv, contents->activation)) {
	      p4 = p3;
	      p3 = temp_wm_value;
	      set_wm_item_last_retrieved (gv, temp_wm_value, TRUE);
	      latest_timestamp_but3 = latest_timestamp_but2;
	      latest_timestamp_but2 = contents->timestamp;
	    }
	    else {
	      set_wm_item_last_retrieved (gv, temp_wm_value, FALSE);
	    }
	  ...
	  /* contingency if r timestamp is latest but 3 */
	  else if (contents->timestamp > latest_timestamp_but3) {
	    pl_is_integer(pl_arg_get(contents->head, 1), &temp_wm_value);
	    if (item_retrieved_from_wm(gv, contents->activation)) {
	      p4 = temp_wm_value;
	      set_wm_item_last_retrieved (gv, temp_wm_value, TRUE);
	      latest_timestamp_but3 = contents->timestamp;
	    }
	    else {
	      set_wm_item_last_retrieved (gv, temp_wm_value, FALSE);
	    }
	...
	if (p1 >= 0) {
	  g1 = (r - p1);
	  p1_latency = wm_retrieval_latency(gv, p1);
	}
	if (p2 >= 0) {
	  g2 = (p1 - p2);
	  p2_latency = wm_retrieval_latency(gv, p2);
	}
        if (p3 >= 0) {
	  g3 = (p2 - p3);
	  p3_latency = wm_retrieval_latency(gv, p3);
	}
        if (p4 >= 0) {
	  g4 = (p3 - p4);
	  p4_latency = wm_retrieval_latency(gv, p4);
	}
	...
	// g2 = (p1 - p2);
	      ...
	      /* -------------- CHECK AFTER p1_latency CYCLES  --------------- */
	      if (gv->cycle - response_buffer_timestamp >= p1_latency) {
		if (g1 == 1) {
		...
		/* ----------- CHECK AFTER p2_latency CYCLES --------------- */
	      
		if (gv->cycle - response_buffer_timestamp >= p2_latency) {
		  if ((g1 > -1) && (g1 == g2)) {
		    // printf ("monitoring: g1 == g2, after %4.2f cycles\n", p2_latency); // debug
		    return (FALSE);
		  }
		}

                /* ----------- CHECK AFTER p3_latency-------------- */
		if (gv->cycle - response_buffer_timestamp >= p3_latency) {
		  if ((g1 > -1) && (g1 == g3)) {
		    // printf ("monitoring: g1 == g3, after %4.2f cycles\n", p3_latency); // debug
		    return (FALSE);
		  }
		}


                /* ----------- CHECK AFTER p4_latency-------------- */
		if (gv->cycle - response_buffer_timestamp >= p4_latency) {
		  if ((g1 > -1) && (g1 == g4)) {
		    // printf ("monitoring: g1 == g4, after %4.2f cycles\n", p4_latency); // debug
		    return (FALSE);
		  }
		}
...
void rng_initialise_model(OosVars *gv)
{
    ...
    /* initialise last_retrieval_success flags */
    /* init should be done by timestamped_clause_list_prepend_element */
    /*
    TimestampedClauseList *contents = NULL;
    contents = oos_buffer_get_contents(gv, BOX_WORKING_MEMORY);
    
    while (!(contents == NULL)) {
      contents->last_retrieval_success = FALSE;
      contents->last_retrieval_timestamp = 0;
      contents = contents->tail;
    }
    */
